...
static void update_recent_cpu_for_all_threads_mlfqs();

static int update_load_avg_mlfqs();

static void update_threads_priorities_mlfqs();

static void update_recent_cpu(struct thread *thread);

static int update_thread_priority_mlfqs(struct thread *thread);
...
thread_tick (void)
{
  ...
  t->recent_cpu = add_int_to_fp(t->recent_cpu, 1);

  if(thread_mlfqs){
    if(timer_ticks () % TIMER_FREQ == 0){
        update_load_avg_mlfqs();

        update_recent_cpu_for_all_threads_mlfqs();
    }

    if(timer_ticks() % TIME_SLICE == 0) {

      update_threads_priorities_mlfqs();
    }
  }
...
thread_set_nice (int nice UNUSED)
{
  ...
  if(!intr_context() && thread_current()->priority < highest_priority) thread_yield();
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  if(t == initial_thread) {
    t->niceness = 0;			/* Initialize Thread nice value. */
    t->recent_cpu = 0;	/* Initialize Thread recent cpu value. */
  } else {
    t->niceness = thread_current()->niceness;			/* Initialize Thread nice value. */
    t->recent_cpu = thread_current()->recent_cpu;	/* Initialize Thread recent cpu value. */
  }

  if(thread_mlfqs && t != idle_thread) {
    update_thread_priority_mlfqs(t);
  }
...
next_thread_to_run (void)
{
  if (list_empty (&ready_list))
    ...
    //return list_entry(list_pop_front(&highest_priority_queue_mlfqs), struct thread, highes_pq_elem);
...
schedule (void)
{
  if(thread_mlfqs) update_threads_priorities_mlfqs();
...
/*static void update_highest_priority_queue() {
  if(!thread_mlfqs)
    return;

  empty_list(&highest_priority_queue_mlfqs);
  ...
  for (listElem = list_begin (&ready_list); listElem != list_end (&ready_list);
       listElem = list_next (listElem))
  {
    ...
    if(thread->priority == highest_priority){
      list_push_back(&highest_priority_queue_mlfqs, &thread->highes_pq_elem);
    }
  }
}*/
...
static void update_threads_priorities_mlfqs() {
  ...
  for (listElem = list_begin (&ready_list); listElem != list_end (&ready_list);
       listElem = list_next (listElem))
  {
    int priority = update_thread_priority_mlfqs(list_entry (listElem, struct thread, elem));
...
static int update_thread_priority_mlfqs(struct thread *thread) {
  ...
  int priority = convert_fp_to_int_rounding(fp_priority);
...
static int update_load_avg_mlfqs() {
  ...
  int second_term = divide_fp_by_int(convert_int_to_fp(ready_threads), 60);
  int first_term = multiply_fp(divide_fp_by_int(convert_int_to_fp(59), 60), load_avg);
...
static void update_recent_cpu(struct thread *thread) {
  ...
  int fp_load_avg_fraction = divide_fp(fp_twice_load_avg, add_int_to_fp(1, fp_twice_load_avg));
...
static void empty_list(struct list *list) {
