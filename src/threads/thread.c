...
void update_threads_priorities_mlfqs() {
...
int update_thread_priority_mlfqs(struct thread *thread) {
    int priority = convert_fp_to_int_rounding(add_int_to_fp(PRI_MAX - (thread->niceness * 2),
                                                            multiply_int_by_fp(-1, divide_fp_by_int(thread->recent_cpu,
                                                                                                    4))));
...
int update_load_avg_mlfqs() {
    ...
    load_avg = add_fp(multiply_fp(divide_fp_by_int(convert_int_to_fp(59), 60), load_avg),
                      divide_fp_by_int(convert_int_to_fp(ready_threads), 60));
...
static void update_recent_cpu(struct thread *thread) {
    ...
    thread->recent_cpu = add_int_to_fp(multiply_fp(divide_fp(fp_twice_load_avg, add_int_to_fp(1, fp_twice_load_avg)),
                                                   thread->recent_cpu), thread->niceness);
}
